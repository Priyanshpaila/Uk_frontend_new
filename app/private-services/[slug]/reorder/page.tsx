...
const REORDER_RAF_FORM_ID_KEY = (slug: string) => `reorder_raf_form_id.${slug}`;
...
function extractAssignedRafFormId(service: any): string | undefined {
  ...
  // Look for 'reorder' in the assignment
  const reorderAssignment = parsed?.reorder; // specifically look for 'reorder'

  if (reorderAssignment) {
    const formId = String(reorderAssignment).trim();
    
    // Save the formId in the RAF_FORM_ID_KEY
    if (typeof window !== "undefined") {
      window.localStorage.setItem(REORDER_RAF_FORM_ID_KEY(service?.slug), formId);
      window.sessionStorage.setItem(REORDER_RAF_FORM_ID_KEY(service?.slug), formId);
    }
    
    return formId;
  }

  // Existing array-based fallbacks (ensure 'reorder' type)
  const assignments =
  ...
  // Filter the list to find "reorder" forms
  const reorderForm = list.find((a: any) => {
    const type =
    ...
    const isReorderForm = type === "reorder" || name === "reorder";

    return isReorderForm && a?.active !== false && !!pickId(a);
  });

  if (reorderForm) {
    const formId = pickId(reorderForm);
  ...
  // If not found, return the first active form
      ...
      try {
        ...
        try {
          ...
          if (assignedRafFormId) {
            const existing = safeGetLocal(REORDER_RAF_FORM_ID_KEY(slug));
    ...
    const safe = (cartItems || []).map((ci: any) => ({
      ...
      unitMinor:
        typeof ci.unitMinor === "number"
          ...
          : 0,
      variation:
        ci.variation || ci.variations || ci.optionLabel || ci.label || "",
    }));
    safe.sort((a, b) =>
      (a.sku + a.variation).localeCompare(b.sku + b.variation)
    );
  ...
  const handleNext = async () => {
    ...
    if (
      ...
      try {
        ...
        toast.error(
          e?.message || "Unable to prepare your order. Please try again."
        );
        return;
      }
    }
